...
void CNode::RecvMsg(char *buf, int32_t buf_len) {
    ...
    if (buf_len < 0) {
        ...
        if (nErr != WSAEWOULDBLOCK && nErr != WSAEMSGSIZE && nErr != WSAEINTR && nErr != WSAEINPROGRESS) {
            if (!fDisconnect) {
                LogPrint("net", "Socket recv failed with: (%d)\n", nErr);
            }
            CloseSocketDisconnect();
        }
        return;
            ...
            if (FD_ISSET(pnode->hSocket, &fdsetRecv) || FD_ISSET(pnode->hSocket, &fdsetError))
            {
                ...
                if (lockRecv)
                {
                    if (pnode->GetTotalRecvSize() > ReceiveFloodSize()) {
                        ...
                        char pchBuf[0x10000];
                        pnode->RecvMsg(pchBuf, sizeof(pchBuf));
                    }
                }
            }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Ember " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Ember is probably already running."), addrBind.ToString());
        else
